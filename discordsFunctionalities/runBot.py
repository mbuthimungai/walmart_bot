from .sendMessages import menu, getDataByProductSearch, getProductDataCategory
import discord
import os, asyncio
import re
...
def find_and_print_channel_by_name(channel_name):
    """
    Find a channel by its name in the specified guild and print its details.

    :param channel_name: The name of the channel to find.
    """
    YOUR_SERVER_ID = os.getenv("SERVER_ID")  # Replace with your server's ID
    ...
    if guild:
        for channel in guild.channels:
            if channel.name == channel_name:
                ...
                return channel
    print('Channel not found or guild not found.')
...
def run_discord_bot():
    ...
    async def on_message(message):
        ...
        search_channel_pattern = r"""^!search\s+https:\/\/www\.walmart\.com\/[^\s]+\s+!channel\s+\S+$"""
            ...
            if message.guild and re.search(search_channel_pattern, user_message):
                ...
                if message.author.guild_permissions.manage_channels:
                    ...
                    if channel:
                        ...
                        channel = discord.utils.get(message.guild.channels, name=channel_name)                                                            
                    await getDataByProductSearch(scheduler, search_msg, channel)
